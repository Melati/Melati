...
public class ContactCategoryTableTest extends PoemTestCase {
  ...
  public void testEnsure() {
    ...
    c.setName("Cat");
    c.makePersistent();
    Contact contact = ((ContactsDatabase) getDb()).getContactTable().ensure("Cat", null, "");
    assertFalse(contact.isIn(c));

    ContactCategory cc = ((ContactsDatabase) getDb()).getContactCategoryTable().ensure(contact, c);
    assertNotNull(cc);
    // Repeat to cover both paths
    ContactCategory cc2 = ((ContactsDatabase) getDb()).getContactCategoryTable().ensure(contact, c);
    assertNotNull(cc2);

    assertTrue(contact.isIn(c));
